...
# Admins & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5243845210').split()] # For Multiple Id Use One Space Between Each.
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '5243845210').split()]  # For Multiple Id Use One Space Between Each.
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', ' -1002194586729').split()]  # For Multiple Id Use One Space Between Each.
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002489988767')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002390793402')).split()]  # For Multiple Id Use One Space Between Each.
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://DearDevil:DearDevil@deardevil.npqq7.mongodb.net/?retryWrites=true&w=majority&appName=DearDevil")   # IF Multiple Database Is False Then Fill Only This Database Url.
DATABASE_NAME = environ.get('DATABASE_NAME', "DearDevil")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'DearDevil')
...
# Links
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+-C2UXurGEmo0MDY9')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/+-ymH_Zz_UVBjNDY1')
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'D_HOUSE99') # Support Chat Link Without https:// or @
